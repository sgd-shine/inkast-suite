...
/** Convert a webcam size percentage (10–100) to a fraction of the reference dimension.
 *  Ceiling raised 50→100 so the webcam can be made prominent (talking-head ~75% width on
 *  16:9 at 100);预设 ≤50 渲染结果与改前完全一致(向后兼容,旧工程不受影响)。 */
...
export function computeCompositeLayout(params: {
	...
	if (webcamPosition) {
		// Custom position: cx/cy 是 webcam 中心相对画布的归一化坐标。落点公式抽成
		// placeWebcamCenter,供 follow 预览每帧复用 → 预览与导出同源,不重复实现。
		const placed = placeWebcamCenter({ width, height }, webcamPosition, {
			width: canvasWidth,
			height: canvasHeight,
		});
		webcamX = placed.x;
...
/**
 * 给定已算好尺寸的 webcam rect 和归一化中心,算它在画布里的左上落点(钳制在画布内)。
 * computeCompositeLayout 的 picture-in-picture 分支用它定位;follow 预览每帧用同一公式
 * 按当前时间的插值位置重算落点 → 预览与导出完全同源。
 */
export function placeWebcamCenter(
	...
	const x = Math.max(
		0,
		Math.min(canvas.width - size.width, Math.round(center.cx * canvas.width - size.width / 2)),
	);
	const y = Math.max(
		0,
		Math.min(canvas.height - size.height, Math.round(center.cy * canvas.height - size.height / 2)),
	);
	return { x, y };
}
