...
export function getSourceCopyFastPathBlockers(
	...
	if (hasActiveTimeRegions(config.overlayClipRegions)) blockers.push("overlay clips are present");
	if (config.showSubtitles && (config.subtitleCues?.length ?? 0) > 0)
		blockers.push("subtitles are present");
...
export class VideoExporter {
	...
	// Inkast §3 Phase 2:导出期间加载的叠加视频 <video>,导出结束后释放。
	private overlayClipVideos: Map<string, HTMLVideoElement> | null = null;
	...
	private async exportWithEncoderPreference(
		...
		try {
			...
			// Inkast §3 Phase 2:预加载每个视频片段叠加层的 <video>(导出逐帧 seek 取样用)。
			const overlayClipVideos = new Map<string, HTMLVideoElement>();
			if (this.config.overlayClipRegions && this.config.overlayClipRegions.length > 0) {
				await Promise.all(
					this.config.overlayClipRegions.map(async (clip) => {
						const v = document.createElement("video");
						v.src = toFileUrl(clip.sourcePath);
						v.muted = true;
						v.preload = "auto";
						v.playsInline = true;
						...
						overlayClipVideos.set(clip.id, v);
					}),
				);
			}
			this.overlayClipVideos = overlayClipVideos;
			...
			const renderer = new FrameRenderer({
				...
				webcamPositionMode: this.config.webcamPositionMode,
				webcamPositionTimeline: this.config.webcamPositionTimeline,
				annotationRegions: this.config.annotationRegions,
				overlayClipRegions: this.config.overlayClipRegions,
				overlayClipVideos,
				...
				zoomCursorTelemetry: this.config.zoomCursorTelemetry,
	...
	private cleanup(): void {
		...
		// Inkast §3 Phase 2:释放叠加视频 <video>。
		if (this.overlayClipVideos) {
			for (const v of this.overlayClipVideos.values()) {
				try {
					v.pause();
