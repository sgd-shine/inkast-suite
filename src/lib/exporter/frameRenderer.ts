...
import {
	...
	smoothAutoFollowFocus,
} from "@/components/video-editor/videoPlayback/autoFollowSmoothing";
import {
	DEFAULT_FOCUS,
	ZOOM_SCALE_DEADZONE,
...
} from "@/components/video-editor/videoPlayback/webcamZoomSync";
...
import type { CameraPositionMode, WebcamPositionSample } from "@/lib/recordingSession";
...
interface FrameRenderConfig {
	...
	// 摄像头位置模式:follow + 时间线时,导出按当前帧时间取插值位置(成片还原拖动轨迹)。
	webcamPositionMode?: CameraPositionMode;
	webcamPositionTimeline?: WebcamPositionSample[];
	annotationRegions?: AnnotationRegion[];
	// Inkast §3 Phase 2:视频片段叠加层 + 各自已加载的 <video>(由 videoExporter 提供,
	// 导出时按 source 时间 seek 后逐帧合成)。
	overlayClipRegions?: import("@/components/video-editor/types").OverlayClipRegion[];
	overlayClipVideos?: Map<string, HTMLVideoElement>;
	// Inkast §4 Phase 3:烧进导出的字幕(源时间轴 cue),底部居中。
	subtitleCues?: import("@/components/video-editor/types").SubtitleCue[];
	showSubtitles?: boolean;
	speedRegions?: SpeedRegion[];
	previewWidth?: number;
	previewHeight?: number;
	cursorTelemetry?: import("@/components/video-editor/types").CursorTelemetryPoint[];
	zoomCursorTelemetry?: import("@/components/video-editor/types").CursorTelemetryPoint[];
...
// Inkast §3 Phase 2:把一个 <video> seek 到指定时间并等到 'seeked'(带安全超时),
// 用于导出时逐帧取叠加视频片段的画面。
function seekVideoToTime(video: HTMLVideoElement, timeSec: number): Promise<void> {
	return new Promise((resolve) => {
		if (video.readyState >= 2 && Math.abs(video.currentTime - timeSec) < 0.02) {
			resolve();
			return;
		}
		let done = false;
		...
		// Safety: never hang the export if a seek never fires.
...
export class FrameRenderer {
	...
	private smoothedAutoFocus: AutoFollowState | null = null;
	...
	async renderFrame(
		...
		// Inkast §3 Phase 2:在标注之上合成视频片段叠加层(画中画)。
		await this.drawOverlayClips(timeMs);
		...
		// Inkast §4 Phase 3:字幕画在最终合成画面之上(最顶层、不随 3D 旋转)。
		this.drawSubtitles(timeMs);
	}

	// Inkast §4 Phase 3:在 compositeCanvas 上绘制当前时间的字幕(底部居中,深色药丸底)。
	// 比例与预览 SubtitleOverlay 一致:字号≈高 4.5%、底边距≈7%、最大宽 82%。
	private drawSubtitles(timeMs: number): void {
		const ctx = this.compositeCtx;
		if (!ctx || this.config.showSubtitles !== true) return;
		const cues = this.config.subtitleCues;
		if (!cues || cues.length === 0) return;
		const cue = activeCueAt(cues, timeMs);
		if (!cue) return;
		...
		const maxTextWidth = Math.round(W * 0.82) - padX * 2;

		ctx.save();
		ctx.font = `600 ${fontSize}px -apple-system, "PingFang SC", "Microsoft YaHei", system-ui, sans-serif`;
		...
		let boxWidth = 0;
		for (const ln of lines) boxWidth = Math.max(boxWidth, ctx.measureText(ln).width);
		boxWidth = Math.min(boxWidth, maxTextWidth);

		const boxH = lines.length * lineHeight + padY * 2;
		const boxW = boxWidth + padX * 2;
		const boxX = Math.round((W - boxW) / 2);
		const boxY = Math.round(H - H * 0.07 - boxH);
		const radius = Math.round(fontSize * 0.32);
		...
		let textY = boxY + padY + Math.round(fontSize * 0.85);
		for (const ln of lines) {
			ctx.fillText(ln, centerX, textY);
			textY += lineHeight;
		}
		ctx.restore();
	}
	...
	// Inkast §3 Phase 2:把当前时间内的视频片段叠加层合成到前景画布(标注之上)。
	// 每个片段把它的 <video> seek 到 source 时间后,按 object-cover 画进它的框、套不透明度。
	private async drawOverlayClips(timeMs: number): Promise<void> {
		const clips = this.config.overlayClipRegions;
		const videos = this.config.overlayClipVideos;
		const ctx = this.foregroundCtx;
		if (!clips || clips.length === 0 || !videos || !ctx) return;
		...
		for (const clip of active) {
			...
			if (!video) continue;
			const clipSourceSec = Math.max(0, (clip.sourceStartMs + (timeMs - clip.startMs)) / 1000);
			await seekVideoToTime(video, clipSourceSec);
			const vw = video.videoWidth;
			const vh = video.videoHeight;
			if (!vw || !vh) continue;

			const boxX = (clip.position.x / 100) * W;
			const boxY = (clip.position.y / 100) * H;
			const boxW = (clip.size.width / 100) * W;
			const boxH = (clip.size.height / 100) * H;
			// object-cover:缩放铺满框,裁掉溢出。
			...
			try {
				...
				// video not ready for this frame — skip rather than abort export.
			}
			ctx.restore();
		}
	}
	...
	private updateLayout(webcamFrame?: VideoFrame | null): void {
		...
		const compositeLayout = computeCompositeLayout({
			...
			// follow + 时间线 → 按当前帧(源视频)时间插值;否则用静态位置(向后兼容)。
			webcamPosition: resolveWebcamPositionAt({
				mode: this.config.webcamPositionMode,
				timeline: this.config.webcamPositionTimeline,
				staticPosition: this.config.webcamPosition,
	...
	private updateAnimationState(timeMs: number): number {
		...
		const { region, strength, blendedScale, rotation3D, transition } = findDominantRegion(
			...
			{
				connectZooms: true,
				cursorTelemetry: this.config.zoomCursorTelemetry ?? this.config.cursorTelemetry,
			},
		);
		...
		if (region && strength > 0) {
			...
			if (region.focusMode === "auto" && !transition) {
				...
				const smoothed = smoothAutoFollowFocus({
					raw,
					previous: this.smoothedAutoFocus,
					deltaMs: dtMs > 0 ? dtMs : 1000 / 60,
					zoomScale,
				});
	...
	private compositeWithShadows(
		...
		const layoutCache = this.layoutCache;
		const baseWebcamRect = layoutCache?.webcamRect ?? null;
		const webcamRect =
			layoutCache && baseWebcamRect && shouldSyncWebcamWithZoom(this.config.webcamLayoutPreset)
				? projectHudRectForZoomState(
						baseWebcamRect,
						{
