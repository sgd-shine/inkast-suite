...
import { computeCompositeLayout, placeWebcamCenter } from "./compositeLayout";
...
describe("computeCompositeLayout", () => {
	...
	it("clamps webcamSizePreset to the valid range (10–100)", () => {
		const canvasSize = { width: 1920, height: 1080 };
		const screenSize = { width: 1920, height: 1080 };
		const webcamSize = { width: 1280, height: 720 };
		...
		// 上限 100 比上限 50 明显更大(摄像头可做大,治"非常小")。
		const atFifty = computeCompositeLayout({
			canvasSize,
			screenSize,
			webcamSize,
			webcamSizePreset: 50,
		});
		expect(atMax!.webcamRect!.width).toBeGreaterThan(atFifty!.webcamRect!.width);
	});
...
describe("placeWebcamCenter", () => {
	it("centers the webcam at the normalized position", () => {
		const placed = placeWebcamCenter(
			{ width: 100, height: 100 },
			{ cx: 0.5, cy: 0.5 },
			{ width: 1000, height: 1000 },
		);
		expect(placed).toEqual({ x: 450, y: 450 });
	});

	it("clamps to the canvas bounds at the corners", () => {
		const canvas = { width: 1000, height: 800 };
		const size = { width: 100, height: 100 };
		expect(placeWebcamCenter(size, { cx: 1, cy: 1 }, canvas)).toEqual({ x: 900, y: 700 });
		expect(placeWebcamCenter(size, { cx: 0, cy: 0 }, canvas)).toEqual({ x: 0, y: 0 });
	});

	it("matches computeCompositeLayout pip placement so preview equals export", () => {
		// 负责人真实 fixed 录制 position 0.9105/0.1799 → 右上;follow 预览每帧用 placeWebcamCenter
		// 应得到与 computeCompositeLayout(导出)完全一致的落点 → 保证 WYSIWYG。
		const canvas = { width: 1920, height: 1080 };
		const center = { cx: 0.9105263157894737, cy: 0.17985611510791366 };
		const layout = computeCompositeLayout({
			canvasSize: canvas,
			screenSize: canvas,
		...
		const rect = layout!.webcamRect!;
		expect(placeWebcamCenter(rect, center, canvas)).toEqual({ x: rect.x, y: rect.y });
	});
});
