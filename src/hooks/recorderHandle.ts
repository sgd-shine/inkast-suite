...
export type RecorderHandle = {
	 ...
	 * only when the user deliberately discards a recording (cancel/restart), so
	 * cancelled runs don't leak the stream or orphan a partial file. No-op for
	 * in-memory recorders.
	 */
	discard: () => Promise<void>;
	/**
	 * Close the disk stream (if one opened) but KEEP its file. Called when saving
	 * the session failed, so the raw recording survives on disk for manual
	 * recovery instead of being deleted with the stream. Resolves to whether a
	 * file was actually preserved (false for in-memory recorders, which have no
	 * on-disk file to keep).
	 */
	closePreservingFile: (durationMs?: number) => Promise<boolean>;
};
...
export function createRecorderHandle(
	...
	async function closePreservingFile(durationMs?: number): Promise<boolean> {
		if (streamOpened && fileName && api?.finalizeRecordingStream) {
			await api.finalizeRecordingStream(fileName, durationMs);
			return true;
		}
		return false;
	}
