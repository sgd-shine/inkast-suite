...
describe("createRecorderHandle", () => {
	...
	it("closePreservingFile finalizes the stream keeping the file for a streamed recording", async () => {
		const finalizeRecordingStream = vi.fn(async () => ({ success: true, streamed: true }));
		const closeRecordingStream = vi.fn(async () => ({ success: true }));
		stubElectronAPI({
			openRecordingStream: vi.fn(async () => ({ success: true })),
			appendRecordingChunk: vi.fn(async () => ({ success: true })),
		...
		await expect(handle.closePreservingFile(1234)).resolves.toBe(true);
		expect(finalizeRecordingStream).toHaveBeenCalledWith("rec.webm", 1234);
		// Preserving must never route through the deleting close.
		expect(closeRecordingStream).not.toHaveBeenCalled();
	});

	it("closePreservingFile reports no file kept when the stream never opened", async () => {
		const finalizeRecordingStream = vi.fn(async () => ({ success: true, streamed: false }));
		stubElectronAPI({
			openRecordingStream: vi.fn(async () => ({ success: false })),
			appendRecordingChunk: vi.fn(async () => ({ success: true })),
			finalizeRecordingStream,
		...
		await expect(handle.closePreservingFile(1234)).resolves.toBe(false);
		expect(finalizeRecordingStream).not.toHaveBeenCalled();
	});

	it("closePreservingFile reports no file kept when finalizeRecordingStream is unavailable (version skew)", async () => {
		stubElectronAPI({
			openRecordingStream: vi.fn(async () => ({ success: true })),
			appendRecordingChunk: vi.fn(async () => ({ success: true })),
			// finalizeRecordingStream intentionally omitted to simulate renderer/main skew.
		});
