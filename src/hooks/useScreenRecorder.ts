...
export function useScreenRecorder(): UseScreenRecorderReturn {
	...
	const finalizeRecording = useCallback(
		(
			...
			void (async () => {
				// Each disk stream must end up saved, preserved, or explicitly
				// discarded. store-recorded-session finalizes the streams included in a
				// successful save; a FAILED save preserves the on-disk screen recording
				// for manual recovery (only a deliberate user discard deletes it); the
				// finally block discards everything else.
				...
				try {
					...
					if (!result.success) {
						console.error("Failed to store recording session:", result.message, result.error);
						saveFailed = true;
						return;
					}
					...
					// A deliberately discarded run can also land here (its blob promise
					// rejects on a mid-stream write error) — that is still a discard,
					// not a failed save, so don't preserve the file for it.
					saveFailed = discardRecordingId.current !== activeRecordingId;
				} finally {
					if (saveFailed) {
						// The save failed: keep the on-disk screen recording so the user
						// can recover it manually; only flush and close its stream. Tell
						// the user — a silent failure here loses the whole take.
						const fileKept = await activeScreenRecorder
							.closePreservingFile(duration)
							.catch(() => false);
						toast.error(t(fileKept ? "recording.saveFailedFileKept" : "recording.saveFailed"));
					} else if (!storeSucceeded) {
						// Deliberate discard (cancel/restart) or nothing to save — delete
						// the partial file so cancelled runs don't orphan it.
						await activeScreenRecorder.discard().catch(() => undefined);
					}
					// The webcam sidecar stays disposable: discard it unless it was part
					// of a successful save (the screen recording is the irreplaceable
					// part; native finalize paths re-read the webcam from its recorder).
		...
		[cursorCaptureMode, teardownMedia, t],
	);
	...
	// Latest-ref pattern: the implementation is rebuilt whenever its inputs change
	// and the ref is re-pointed by the effect below, so stopRecording.current()
	// always runs a closure that sees current state (cursorCaptureMode via the
	// finalize callbacks). A bare useRef(fn) would freeze the first render's
	// closure forever and stamp stale metadata into every saved session.
	...
	const stopRecording = useRef(stopRecordingImpl);
	useEffect(() => {
		stopRecording.current = stopRecordingImpl;
	}, [stopRecordingImpl]);
	...
	const startRecording = async (countdownRunToken?: number) => {
		try {
			...
			// Inkast: skip the native ScreenCaptureKit helper on macOS so capture runs
			// through Electron's getDisplayMedia/getUserMedia below — i.e. in Inkast's
			// OWN process. The helper is a separate process and thus a distinct macOS
			// "Screen Recording" TCC subject; for self-signed / non-notarized local
			// builds macOS won't group it with Inkast nor remember its grant, so it
			// re-prompts on every recording. Routing through the app makes the grant
			// stick. Set this to false to restore the native path (richer cursor
			// metadata for the editor) once the app is properly signed + notarized.
			const FORCE_GET_DISPLAY_MEDIA_ON_MAC = true;
			if (
				!FORCE_GET_DISPLAY_MEDIA_ON_MAC &&
				(await startNativeMacRecordingIfAvailable(selectedSource, countdownRunToken))
			) {
				return;
			}
			...
			if (platform === "win32" || platform === "darwin") {
				...
				// pre-selected source (useSystemPicker:false → no system chooser dialog).
				//
				// win32: cursor:"never" 真生效 → 系统光标被排除 → 编辑器叠加替身光标
				//        (见 VideoEditor.hasEditableCursorRecording)。
				//
				// darwin (Inkast / Screen-Studio 干净光标):mac 也走这条分支,因为它是
				//   App 进程内捕获(权限随 D10 持久、不启用原生 helper、无新录屏弹窗),
				//   且与 win32 统一、相比旧 getUserMedia 路径无回归。
				//   ⚠️ 已于 2026-06-17 真机验证:macOS 的 getDisplayMedia(ScreenCaptureKit
				//   后端)**忽略 cursor 约束** —— 抠录像帧确认系统光标仍被烙进视频(见
				//   DECISIONS D15)。所以 cursor:"never" 在 mac 是无害 no-op,系统光标排不掉;
				//   因此 VideoEditor 在 mac **不叠加**替身光标(D13)→ 画面只有这一个真光标、
				//   无重影。要"无系统光标的视频"只能上原生 SCK showsCursor=false(路线 B,
				//   会带回权限弹窗,负责人 2026-06-17 选择暂不做)。
				//
				// 系统音频:loopback 内录在 Electron 41 仅 Windows 由 handler 提供;mac
				//   getDisplayMedia 不带系统内录声 —— 与切换前一致(旧 chromeMediaSource
				//   desktop 音频在 mac 本就不可用),不崩溃,麦克风不受影响。
				...
				// 其它平台(linux 等)仍走 legacy getUserMedia + chromeMediaSource 桌面采集。
				// 该 API 没有 cursor 选项 → 系统光标会被烙进视频(这些平台暂不做干净光标)。
