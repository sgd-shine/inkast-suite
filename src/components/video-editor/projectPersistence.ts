...
export function normalizeProjectEditor(editor: Partial<ProjectEditorState>): ProjectEditorState {
		...
		? editor.annotationRegions
				...
				.map((region, index) => {
					...
					return {
						...
						// Inkast §3 图片叠加:保留不透明度(0–1),undefined 表示完全不透明。
						opacity: isFiniteNumber(region.opacity) ? clamp(region.opacity, 0, 1) : undefined,
	...
	// Inkast §3 Phase 2:视频片段叠加层归一化(只接受带合法源路径与时间区间的条目)。
	const normalizedOverlayClipRegions: OverlayClipRegion[] = Array.isArray(editor.overlayClipRegions)
		? editor.overlayClipRegions
				.filter((region): region is OverlayClipRegion =>
					Boolean(
						region &&
							typeof region.id === "string" &&
							typeof region.sourcePath === "string" &&
							region.sourcePath.trim().length > 0,
					),
				)
				.map((region, index) => {
					const rawStart = isFiniteNumber(region.startMs) ? Math.round(region.startMs) : 0;
					...
					return {
						...
						sourceStartMs: isFiniteNumber(region.sourceStartMs)
							...
							: 0,
						sourceDurationMs: isFiniteNumber(region.sourceDurationMs)
							? Math.max(0, Math.round(region.sourceDurationMs))
							: undefined,
						position: {
							x: clamp(isFiniteNumber(region.position?.x) ? region.position.x : 34, 0, 100),
							y: clamp(isFiniteNumber(region.position?.y) ? region.position.y : 38, 0, 100),
						},
						size: {
							width: clamp(isFiniteNumber(region.size?.width) ? region.size.width : 32, 1, 200),
							height: clamp(isFiniteNumber(region.size?.height) ? region.size.height : 24, 1, 200),
						},
						opacity: isFiniteNumber(region.opacity) ? clamp(region.opacity, 0, 1) : undefined,
						zIndex: isFiniteNumber(region.zIndex) ? region.zIndex : index + 1,
						muted: region.muted !== false,
					};
				})
		: [];

	// Inkast §4 Phase 3:字幕 cue 归一化(只接受带合法时间区间与非空文本的条目)。
	const normalizedSubtitleCues: SubtitleCue[] = Array.isArray(editor.subtitleCues)
		? editor.subtitleCues
				.filter((cue): cue is SubtitleCue =>
					Boolean(cue && typeof cue.text === "string" && cue.text.trim().length > 0),
				)
				.map((cue, index) => {
					...
					const startMs = Math.max(0, Math.min(rawStart, rawEnd));
					return {
						id: typeof cue.id === "string" ? cue.id : `sub-${index}`,
						startMs,
	...
	return {
		...
		overlayClipRegions: normalizedOverlayClipRegions,
		subtitleCues: normalizedSubtitleCues,
		showSubtitles: editor.showSubtitles === true,
		...
		...(editor.webcamPositionMode === "fixed" || editor.webcamPositionMode === "follow"
			? { webcamPositionMode: editor.webcamPositionMode }
			: {}),
		...(Array.isArray(editor.webcamPositionTimeline) && editor.webcamPositionTimeline.length > 0
			? {
					webcamPositionTimeline: editor.webcamPositionTimeline
						.filter(
							(s): s is WebcamPositionSample =>
								!!s &&
								isFiniteNumber((s as WebcamPositionSample).tMs) &&
								isFiniteNumber((s as WebcamPositionSample).cx) &&
								isFiniteNumber((s as WebcamPositionSample).cy),
						)
						.map((s) => ({
							tMs: Math.max(0, s.tMs),
							cx: clamp(s.cx, 0, 1),
							cy: clamp(s.cy, 0, 1),
						})),
				}
