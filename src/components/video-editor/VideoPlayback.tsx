...
import { resolveWebcamPositionAt } from "@/lib/webcamFollow";
...
import { OverlayClipLayer } from "./OverlayClipLayer";
...
import { type AutoFollowState, smoothAutoFollowFocus } from "./videoPlayback/autoFollowSmoothing";
import {
	DEFAULT_FOCUS,
	ZOOM_SCALE_DEADZONE,
...
import {
	projectHudRectForZoomState,
	shouldSyncWebcamWithZoom,
} from "./videoPlayback/webcamZoomSync";
...
interface VideoPlaybackProps {
	...
	// follow 模式:录制中拖动轨迹,预览按当前时间插值还原(与导出 frameRenderer 同源)。
	webcamPositionMode?: import("@/lib/recordingSession").CameraPositionMode;
	webcamPositionTimeline?: import("@/lib/recordingSession").WebcamPositionSample[];
	...
	// Inkast §3 Phase 2: 视频片段叠加层
	overlayClipRegions?: import("./types").OverlayClipRegion[];
	selectedOverlayClipId?: string | null;
	// Inkast §4 Phase 3: 字幕
	subtitleCues?: import("./types").SubtitleCue[];
	showSubtitles?: boolean;
	onSelectOverlayClip?: (id: string) => void;
	onOverlayClipPositionChange?: (id: string, position: { x: number; y: number }) => void;
	onOverlayClipSizeChange?: (id: string, size: { width: number; height: number }) => void;
	...
	zoomCursorTelemetry?: CursorTelemetryPoint[];
	...
	(
		{
			...
			onSelectOverlayClip,
			onOverlayClipPositionChange,
			onOverlayClipSizeChange,
			subtitleCues = [],
			showSubtitles = false,
			blurRegions = [],
			selectedBlurId,
		...
		const webcamLayoutPresetRef = useRef<WebcamLayoutPreset>(webcamLayoutPreset);
		// follow 预览逐帧化:每帧渲染循环按 currentTime 从这俩 ref 插值 webcam 落点。
		const webcamPositionRef = useRef(webcamPosition);
		const webcamPositionModeRef = useRef(webcamPositionMode);
		const webcamPositionTimelineRef = useRef(webcamPositionTimeline);
		...
		const zoomCursorTelemetryRef = useRef<CursorTelemetryPoint[]>(
			zoomCursorTelemetry ?? cursorTelemetry,
		);
		...
		const smoothedAutoFocusRef = useRef<AutoFollowState | null>(null);
		const prevAutoFollowTimeMsRef = useRef<number | null>(null);
		...
		const handleWebcamPointerDown = (event: React.PointerEvent<HTMLVideoElement>) => {
			...
			// follow:位置由录制轨迹逐帧驱动,禁手动拖动(否则拖动会被下一帧覆盖、弹回)。
			if (webcamPositionMode === "follow" && (webcamPositionTimeline?.length ?? 0) > 0) return;
			event.preventDefault();
		...
		useEffect(() => {
			zoomCursorTelemetryRef.current = zoomCursorTelemetry ?? cursorTelemetry;
		}, [zoomCursorTelemetry, cursorTelemetry]);
		...
		useEffect(() => {
			...
			webcamPositionModeRef.current = webcamPositionMode;
			webcamPositionTimelineRef.current = webcamPositionTimeline;
		}, [webcamPosition, webcamPositionMode, webcamPositionTimeline]);
		...
		useEffect(() => {
			...
			const ticker = () => {
				const { region, strength, blendedScale, rotation3D, transition } = findDominantRegion(
					...
					{
						connectZooms: true,
						cursorTelemetry: zoomCursorTelemetryRef.current,
					},
				);
				...
				if (region && strength > 0 && !shouldShowUnzoomedView) {
					...
					if (region.focusMode === "auto" && !transition) {
						...
						const previousTimeMs = prevAutoFollowTimeMsRef.current;
						const hasTimeDiscontinuity =
							previousTimeMs !== null && (timeMs < previousTimeMs || timeMs - previousTimeMs > 250);
						const smoothed = smoothAutoFollowFocus({
							raw,
							previous: hasTimeDiscontinuity ? null : smoothedAutoFocusRef.current,
							zoomScale,
							deltaMs: previousTimeMs !== null ? Math.max(0, timeMs - previousTimeMs) : 1000 / 60,
						});
						smoothedAutoFocusRef.current = smoothed;
						prevAutoFollowTimeMsRef.current = timeMs;
						targetFocus = smoothed.focus;
					} else if (region.focusMode !== "auto") {
						smoothedAutoFocusRef.current = null;
						prevAutoFollowTimeMsRef.current = null;
					}
				...
				const webcamOverlay = webcamOverlayRef.current;
				let baseWebcamLayout = webcamLayoutRef.current;
				if (webcamOverlay && baseWebcamLayout) {
					// follow:按当前帧源视频时间从录制轨迹插值 webcam 中心,逐帧还原录制中拖动
					// (与导出 frameRenderer 同源:resolveWebcamPositionAt + placeWebcamCenter)。
					// fixed / 旧录制无时间线 → 保持静态位置,行为不变。
					if (
						webcamPositionModeRef.current === "follow" &&
						webcamPositionTimelineRef.current &&
						webcamPositionTimelineRef.current.length > 0 &&
						webcamLayoutPresetRef.current === "picture-in-picture"
					) {
						const followPos = resolveWebcamPositionAt({
							mode: "follow",
							timeline: webcamPositionTimelineRef.current,
							staticPosition: webcamPositionRef.current,
							timeMs: currentTimeRef.current,
						});
						if (followPos) {
							const placed = placeWebcamCenter(baseWebcamLayout, followPos, stageSizeRef.current);
							baseWebcamLayout = { ...baseWebcamLayout, x: placed.x, y: placed.y };
						}
					}
					const projectedWebcamLayout = shouldSyncWebcamWithZoom(webcamLayoutPresetRef.current)
						? projectHudRectForZoomState(
								baseWebcamLayout,
								{
									scale: appliedScale,
						...
						: baseWebcamLayout;
					webcamOverlay.style.left = `${projectedWebcamLayout.x}px`;
					webcamOverlay.style.top = `${projectedWebcamLayout.y}px`;
					webcamOverlay.style.width = `${projectedWebcamLayout.width}px`;
					webcamOverlay.style.height = `${projectedWebcamLayout.height}px`;
					webcamOverlay.style.setProperty(
						"--webcam-radius",
						`${projectedWebcamLayout.borderRadius}px`,
					);
				}
				...
				>
									...
									style={
										{
											...
											"--webcam-radius": `${webcamLayout?.borderRadius ?? 0}px`,
											zIndex: 20,
								...
								>
									<video
										...
										className={`w-full h-full object-cover ${
											...
											!(
												webcamPositionMode === "follow" && (webcamPositionTimeline?.length ?? 0) > 0
											)
												? "cursor-grab active:cursor-grabbing"
												: "pointer-events-none"
										}`}
										style={{
											borderRadius: useClipPath ? 0 : "var(--webcam-radius)",
					...
					{pixiReady && videoReady && (
						...
						>
							...
							{/* Inkast §3 Phase 2: 视频片段叠加层(画中画) */}
							<OverlayClipLayer
								regions={overlayClipRegions}
								containerWidth={overlaySize.width}
								containerHeight={overlaySize.height}
								currentTimeMs={Math.round(currentTime * 1000)}
								isPlaying={isPlaying}
								selectedId={selectedOverlayClipId ?? null}
								onSelect={(id) => onSelectOverlayClip?.(id)}
								onPositionChange={(id, p) => onOverlayClipPositionChange?.(id, p)}
								onSizeChange={(id, s) => onOverlayClipSizeChange?.(id, s)}
							/>
							{/* Inkast §4 Phase 3: 字幕层(底部居中,内容保护无关,仅预览/导出) */}
							<SubtitleOverlay
								cues={subtitleCues}
								currentTimeMs={Math.round(currentTime * 1000)}
								show={showSubtitles}
