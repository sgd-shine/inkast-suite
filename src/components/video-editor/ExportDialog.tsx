...
import { ExportSettingsControls, type ExportSettingsControlsProps } from "./ExportSettingsControls";
...
interface ExportDialogProps {
	...
	onShowInFolder?: () => void;
	// Inkast 布局优化:导出配置(格式/画质/GIF)收口进弹窗。开始导出前展示这组控件 +「导出」按钮。
	settingsControls?: ExportSettingsControlsProps;
	onStartExport?: () => void;
	canExport?: boolean;
	// 保存到默认位置失败时的兜底:保留导出结果,让用户手动选保存位置。
...
export function ExportDialog({
	...
	// Inkast 布局优化:导出前的「配置」相位 —— 既没在导出、也没有进度/兜底保存/成功态时,
	// 展示格式/画质/GIF 控件 +「导出」按钮(唯一的导出动作入口)。
	const isConfiguring =
		!isExporting && !hasUnsavedExport && !showSuccess && !(progress && progress.percentage >= 100);
	...
	return (
		<>
			...
			/>
			<div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-[60] bg-[#0A0C0E] rounded-2xl shadow-2xl border border-white/10 p-8 w-[90vw] max-w-md animate-in zoom-in-95 duration-200">
					...
					<div className="flex items-center gap-4">
						{showSuccess ? (
							...
							<>
								...
								<div>
									<span className="text-xl font-bold text-slate-200 block">
										{isConfiguring ? t("export.exportTitle") : getTitle()}
									</span>
									<span className="text-sm text-slate-400">
										{isConfiguring ? t("export.chooseSettings") : getStatusMessage()}
									</span>
								</div>
							</>
						)}
				...
				{/* Inkast 布局优化:导出配置相位 —— 格式/画质/GIF 控件 +「导出」按钮。 */}
				{isConfiguring && settingsControls && (
					<div className="space-y-1">
						<ExportSettingsControls {...settingsControls} />
						<Button
							data-testid={getTestId("export-start-button")}
							type="button"
							size="lg"
							disabled={!canExport}
							onClick={onStartExport}
							className="w-full py-5 text-sm font-semibold flex items-center justify-center gap-2 bg-[#34B27B] text-white rounded-xl shadow-lg shadow-[#34B27B]/20 hover:bg-[#3fc98d] hover:scale-[1.01] active:scale-[0.99] transition-all duration-200 disabled:opacity-50 disabled:hover:scale-100"
						>
							<Download className="w-4 h-4" />
							{exportFormat === "gif"
								? tSettings("export.gifButton")
								: tSettings("export.videoButton")}
						</Button>
					</div>
				)}
				...
				{/* 兜底:导出已生成但写默认位置失败 → 让用户手动选保存位置。 */}
				{hasUnsavedExport && !isExporting && (
					<Button
						type="button"
						size="lg"
						onClick={onSaveUnsavedExport}
						className="w-full py-5 text-sm font-semibold flex items-center justify-center gap-2 bg-indigo-500 text-white rounded-xl shadow-lg shadow-indigo-500/20 hover:bg-indigo-500/90 transition-all duration-200"
					>
						<Download className="w-4 h-4" />
						{tSettings("export.chooseSaveLocation")}
					</Button>
				)}
				...
				{isExporting && progress && (
					<div className="space-y-6">
                      ...
                      `}</style>
									...
									<div
										className={cn(
											"h-full bg-[#34B27B] shadow-[0_0_10px_rgba(52,178,123,0.3)] transition-all duration-300 ease-out",
										)}
