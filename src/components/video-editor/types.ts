...
export const DEFAULT_CURSOR_MOTION_BLUR = 0;
...
export interface AnnotationRegion {
	...
	blurData?: BlurData;
	// Inkast §3 图片叠加:叠加层不透明度 0–1(undefined = 1 不透明)。目前用于 image 叠加层。
	opacity?: number;
}
...
// Inkast §3 Phase 2:视频片段叠加层(画中画)。时间轴用源时间(同 zoom/trim/annotation),
// 在 [startMs,endMs] 内可见;叠加视频从 sourceStartMs 处开始按源时间推进播放。
// position/size 是相对画面的百分比(同 annotation 图片叠加)。
export interface OverlayClipRegion {
	id: string;
	startMs: number; // 在(源)时间轴上出现的起点
	endMs: number; // 在(源)时间轴上消失的终点
	sourcePath: string; // 叠加视频的本机文件路径(渲染时转 file:// URL)
	sourceStartMs: number; // 裁剪:从源视频的哪一刻开始取
	sourceDurationMs?: number; // 源视频总时长(可选,用于 clamp)
	position: AnnotationPosition; // 左上角 %(相对画面)
	size: AnnotationSize; // 宽高 %
	opacity?: number; // 不透明度 0–1(undefined=1)
	zIndex: number;
	muted: boolean; // 叠加片段默认静音(避免与主音轨冲突)
}

// Inkast §4 Phase 3:自动字幕。每条 cue 的时间戳在(源)时间轴上,与转写词同坐标系,
// 因此 ripple 删除/变速会随源帧自然生效(被剪掉区间的字幕帧也一并跳过)。
...
export const DEFAULT_OVERLAY_CLIP_SIZE: AnnotationSize = {
	width: 32,
	height: 24,
};

/** 新插入的视频叠加层默认时长(源视频更短时取源时长)。 */
export const DEFAULT_OVERLAY_CLIP_DURATION_MS = 5000;
