...
const OVERLAY_ROW_ID = "row-overlay"; // Inkast §3 Phase 2:视频片段叠加轨
const AUDIO_ROW_ID = "row-audio"; // Inkast 布局优化:音频波形独立轨(不再叠在剪掉轨背景)
...
interface TimelineEditorProps {
	...
	// Inkast §3 图片叠加:触发「插入图片」(打开图片选择器,在播放头插入画中画叠加层)。
	onInsertImageOverlay?: () => void;
	// Inkast §3 Phase 2:触发「插入视频」(打开视频选择器,在播放头插入视频画中画叠加层)。
	onInsertVideoOverlay?: () => void;
	// Inkast §3 Phase 2:视频片段叠加轨
	overlayClipRegions?: OverlayClipRegion[];
	selectedOverlayClipId?: string | null;
	onSelectOverlayClip?: (id: string | null) => void;
	onOverlayClipSpanChange?: (id: string, span: Span) => void;
	...
	// Inkast §2: 音频波形开关从右侧 rail 移到这里(波形显示在时间线上,工具条是它的自然归属)。
...
function TimelineAxis({
	...
	return (
		<div
			className="sticky top-0 z-20 h-9 bg-[#0C0F12] border-b border-white/[0.07] overflow-hidden select-none"
			style={{
...
function Timeline({
	...
	return (
		<div
			...
			className="select-none bg-[#0C0F12] min-h-[190px] relative cursor-pointer group"
		...
		>
			...
			{/* Inkast 布局优化:音频波形独立成一条轨道(不再叠在「剪掉」轨背景上)。
			    由工具条「音频波形」开关控制显隐;开启时占一条专属 lane,不与其他轨重叠。 */}
			{showTrimWaveform && (
				<Row
					id={AUDIO_ROW_ID}
					isEmpty={false}
					icon={<AudioLines className="w-3 h-3" />}
					label={t("trackLabels.audio")}
					background={
						<BackgroundWaveform
							peaks={peaks}
							videoDurationMs={videoDurationMs}
			...
			<Row
				...
				label={t("trackLabels.annotation")}
			>
			...
			<Row
				id={OVERLAY_ROW_ID}
				isEmpty={overlayItems.length === 0}
				hint={t("hints.overlayClip")}
				icon={<FilmIcon className="w-3 h-3" />}
				label={t("trackLabels.overlay")}
			>
				{overlayItems.map((item) => (
					<Item
						...
						rowId={item.rowId}
						span={item.span}
						isSelected={item.id === selectedOverlayClipId}
						onSelect={() => onSelectOverlayClip?.(item.id)}
						variant="annotation"
					>
						{item.label}
					</Item>
...
export default function TimelineEditor({
	...
	selectedAnnotationId,
	onSelectAnnotation,
	onInsertImageOverlay,
	onInsertVideoOverlay,
	overlayClipRegions = [],
	selectedOverlayClipId,
	onSelectOverlayClip,
	onOverlayClipSpanChange,
	blurRegions = [],
	onBlurAdded,
	onBlurSpanChange,
	onBlurDelete,
	...
	const handleSuggestZooms = useCallback(() => {
		...
		const suggestions = selectZoomSuggestions(dwellCandidates, totalMs, { reservedSpans });

		suggestions.forEach((suggestion) => {
			onZoomSuggested(suggestion.span, suggestion.focus);
		});
	...
	// Inkast: auto-run the cursor-follow zoom suggestion ONCE for a freshly
	// recorded clip (telemetry loaded + duration known + no zoom regions yet),
	// so 鼠标跟随 is automatic (Screen Studio-style) instead of needing a button
	// click. A loaded project that already has zoom regions is left untouched.
	const hasAutoSuggestedZoomsRef = useRef(false);
	useEffect(() => {
		if (hasAutoSuggestedZoomsRef.current) return;
		if (!videoDuration || totalMs <= 0) return; // duration not ready yet
		if (cursorTelemetry.length < 2) return; // telemetry not loaded yet
		if (zoomRegions.length > 0) return; // loaded project / user already added zooms
		hasAutoSuggestedZoomsRef.current = true;
		handleSuggestZooms();
	}, [videoDuration, totalMs, cursorTelemetry, zoomRegions, handleSuggestZooms]);
	...
	const timelineItems = useMemo<TimelineRenderItem[]>(() => {
		...
		const overlays: TimelineRenderItem[] = overlayClipRegions.map((region, index) => ({
			id: region.id,
			rowId: OVERLAY_ROW_ID,
			span: { start: region.startMs, end: region.endMs },
			label: t("labels.overlayClipItem", { index: String(index + 1) }),
			variant: "annotation",
		}));

		return [...zooms, ...trims, ...annotations, ...blurs, ...speeds, ...overlays];
	}, [
		zoomRegions,
		trimRegions,
		annotationRegions,
	...
	const handleItemSpanChange = useCallback(
		(id: string, span: Span) => {
			...
			} else if (overlayClipRegions.some((r) => r.id === id)) {
				onOverlayClipSpanChange?.(id, span);
			}
		},
		[
			zoomRegions,
	...
	if (!videoDuration || videoDuration === 0) {
		return (
			<div className="flex-1 flex flex-col items-center justify-center rounded-lg bg-[#0A0C0E] gap-3">
	...
	return (
		<div className="flex-1 min-h-0 flex flex-col bg-[#0A0C0E] overflow-hidden">
			<div className="flex items-center gap-2 px-3 py-1.5 border-b border-white/[0.06] bg-[#08090b]/95 overflow-x-auto custom-scrollbar">
				{/* Inkast 界面对齐 Phase 1:效果按钮组从纯图标 → 图标+文字(对齐 Demo .ct)。 */}
				<div className="flex items-center gap-0.5 rounded-xl border border-white/[0.06] bg-white/[0.025] p-0.5 shrink-0">
					<Button
						onClick={handleAddZoom}
						variant="ghost"
						size="sm"
						className="h-7 gap-1 rounded-lg px-2 text-[11px] font-medium text-slate-400 hover:text-[#34B27B] hover:bg-[#34B27B]/10 transition-all"
					...
					>
						...
						{t("buttons.suggestZoomsLabel")}
					</Button>
					<Button
						onClick={handleAddTrim}
						variant="ghost"
						size="sm"
						className="h-7 gap-1 rounded-lg px-2 text-[11px] font-medium text-slate-400 hover:text-[#ef4444] hover:bg-[#ef4444]/10 transition-all"
					...
					<Button
						...
						variant="ghost"
						size="sm"
						className="h-7 gap-1 rounded-lg px-2 text-[11px] font-medium text-slate-400 hover:text-[#B4A046] hover:bg-[#B4A046]/10 transition-all"
						title={t("buttons.addAnnotation")}
					>
						<MessageSquare className="w-3.5 h-3.5" />
						{t("trackLabels.annotation")}
					</Button>
					<Button
						onClick={handleAddBlur}
						variant="ghost"
						size="sm"
						className="h-7 gap-1 rounded-lg px-2 text-[11px] font-medium text-slate-400 hover:text-[#7dd3fc] hover:bg-[#7dd3fc]/10 transition-all"
					...
					<Button
						...
						variant="ghost"
						size="sm"
						className="h-7 gap-1 rounded-lg px-2 text-[11px] font-medium text-slate-400 hover:text-[#d97706] hover:bg-[#d97706]/10 transition-all"
				...
				{/* Inkast §3: 「插入图片 / 插入视频」叠加层(画中画)—— 普通功能按钮组。 */}
				{(onInsertImageOverlay || onInsertVideoOverlay) && (
					<div className="flex items-center gap-1">
						{onInsertImageOverlay && (
							<Button
								onClick={onInsertImageOverlay}
								variant="ghost"
								size="sm"
								className="h-7 gap-1.5 rounded-lg border border-white/[0.06] bg-white/[0.025] px-2.5 text-[11px] font-medium text-slate-300 hover:bg-white/[0.07] hover:text-slate-100 transition-all"
								title={t("buttons.insertImage")}
							>
								<ImageIcon className="w-4 h-4" />
								{t("buttons.insertImageLabel")}
							</Button>
						)}
						{onInsertVideoOverlay && (
							<Button
								onClick={onInsertVideoOverlay}
								variant="ghost"
								size="sm"
								className="h-7 gap-1.5 rounded-lg border border-white/[0.06] bg-white/[0.025] px-2.5 text-[11px] font-medium text-slate-300 hover:bg-white/[0.07] hover:text-slate-100 transition-all"
								title={t("buttons.insertVideo")}
							>
								<FilmIcon className="w-4 h-4" />
								{t("buttons.insertVideoLabel")}
							</Button>
						)}
					</div>
				)}
				{/* Inkast 布局优化:智能粗剪入口收口到左侧「智能」面板,工具条不再重复放按钮。 */}
				{/* Inkast §2: 音频波形开关(从右侧 rail「时间轴」面板移来,这里是它的自然归属)。 */}
				{onTrimWaveformChange && (
					<button
						type="button"
						onClick={() => onTrimWaveformChange(!showTrimWaveform)}
						title={t("buttons.toggleWaveform")}
						aria-pressed={showTrimWaveform}
						className={cn(
				...
				{/* Inkast 界面对齐 Phase 0:比例下拉已搬到标题栏右侧(对齐 Demo .tb-right)。 */}
			...
			<div
				...
				className="flex-1 min-h-0 overflow-auto custom-scrollbar bg-[#0A0C0E] relative"
			...
			>
				...
				>
					...
					<Timeline
						...
						onSelectOverlayClip={onSelectOverlayClip}
