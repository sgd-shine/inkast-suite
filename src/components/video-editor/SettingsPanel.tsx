...
import { OverlayClipSettingsPanel } from "./OverlayClipSettingsPanel";
import type {
	AnnotationRegion,
	AnnotationType,
	BlurData,
...
interface SettingsPanelProps {
	...
	// Inkast §2: 选中时间线片段时的友好名(如「放大 1」)+ 清除选择回调(用于 Inspector「← 返回全局」)
	...
	// Inkast §3 图片叠加:『画面』数值控件回调(位置/大小/不透明度)。
	onAnnotationPositionChange?: (id: string, position: { x: number; y: number }) => void;
	onAnnotationSizeChange?: (id: string, size: { width: number; height: number }) => void;
	onAnnotationOpacityChange?: (id: string, opacity: number) => void;
	// Inkast §3 Phase 2:视频片段叠加层(选中时显示『画面』面板)。
	selectedOverlayClip?: import("./types").OverlayClipRegion | null;
	...
	onOverlayClipOpacityChange?: (id: string, opacity: number) => void;
	onOverlayClipDelete?: (id: string) => void;
...
type SettingsPanelMode = "background" | "effects" | "layout" | "cursor" | "timeline";
...
export function SettingsPanel({
	...
	const panelModes: Array<{
		...
		label: string;
		// 短标签:显示在窄 rail 图标下方(header 用更清晰的长 label)。
		railLabel: string;
	...
	}> = [
		{
			...
			railLabel: t("background.title"),
			icon: Palette,
		},
		{
			id: "effects",
			label: t("effects.title"),
			railLabel: t("rail.appearance"),
			icon: SlidersHorizontal,
		},
		{
			id: "layout",
			label: t("layout.title"),
			railLabel: t("rail.camera"),
			icon: LayoutPanelTop,
			disabled: !hasWebcam,
		},
		// Inkast §2 UI 清晰化:移除与底部时间线重名的「时间轴」rail 项(其唯一的音频波形
		// 开关已移到底部时间线工具条,见 TimelineEditor)。光标项用专属 cursor.title 标签
		// (原来误用 effects.title)。
		...(hasCursorPanel
			? [
					{
	...
	const activeModeLabel = hasTimelineSelection
		...
		: activePanelMode === "timeline"
			? t("timeline.title")
			: (panelModes.find((mode) => mode.id === activePanelMode)?.label ?? t("background.title"));
	...
	const selectedBlur = selectedBlurId
		...
		: null;
	// Inkast §2: 报告错误 / 保存诊断 / GitHub 加星 等低频项已从面板底部移到顶栏「帮助」菜单
	// (见 VideoEditor 顶栏),让 Inspector 更聚焦当前编辑对象。

	// Inkast §3 Phase 2:选中视频片段叠加层 → 显示叠加层『画面』面板。
	if (
		selectedOverlayClip &&
		onOverlayClipPositionChange &&
		onOverlayClipSizeChange &&
		...
		const clip = selectedOverlayClip;
		return (
			<div className="editor-inspector-shell flex min-w-0 flex-col h-full overflow-hidden">
				<div className="min-h-0 flex-1 overflow-hidden">
					<OverlayClipSettingsPanel
						region={clip}
						onPositionChange={(position) => onOverlayClipPositionChange(clip.id, position)}
						onSizeChange={(size) => onOverlayClipSizeChange(clip.id, size)}
						onOpacityChange={(opacity) => onOverlayClipOpacityChange(clip.id, opacity)}
						onDelete={() => onOverlayClipDelete(clip.id)}
					/>
				</div>
			</div>
		);
			...
			<div className="editor-inspector-shell flex min-w-0 flex-col h-full overflow-hidden">
				<div className="min-h-0 flex-1 overflow-hidden">
					<AnnotationSettingsPanel
						...
						onPositionChange={
							onAnnotationPositionChange
								? (position) => onAnnotationPositionChange(selectedAnnotation.id, position)
								: undefined
						}
						onSizeChange={
							onAnnotationSizeChange
						...
						onOpacityChange={
							onAnnotationOpacityChange
								? (opacity) => onAnnotationOpacityChange(selectedAnnotation.id, opacity)
								: undefined
						}
					/>
				</div>
	...
	return (
		<div className="editor-inspector-shell flex min-w-0 flex-col h-full overflow-hidden">
			<div className="flex min-h-0 flex-1">
				{/* Inkast §2: rail 分「画面 / 项目」两组,每项图标下加文字标签(对照 UI 样板方向 A)。 */}
				<div className="settings-mode-rail flex w-[70px] shrink-0 flex-col gap-0.5 overflow-y-auto border-r border-white/[0.07] bg-black/20 px-1.5 py-2">
					<div className="px-1 pb-0.5 text-center text-[8.5px] font-semibold uppercase tracking-[0.14em] text-slate-600">
						{t("rail.groupScreen")}
					</div>
					{panelModes.map((mode) => {
						...
						return (
							...
							>
								...
								<span className="text-[9.5px] font-medium leading-none">{mode.railLabel}</span>
							</button>
						);
					})}
					<button
						...
						className="flex flex-col items-center gap-1 rounded-lg border border-transparent px-1 py-1.5 text-slate-500 transition-all hover:border-white/10 hover:bg-white/[0.06] hover:text-slate-200"
					>
						<Crop className="h-[18px] w-[18px]" />
						<span className="text-[9.5px] font-medium leading-none">{t("crop.title")}</span>
					</button>
					{/* Inkast 布局优化:导出入口收口到顶栏绿色按钮 + 导出弹窗,这里不再放「导出」rail 项。 */}
				</div>
				<div className="flex-1 overflow-y-auto custom-scrollbar p-3 pb-0">
					{/* Inkast §2: Inspector 固定标题区 —— 全局模式显示当前 tab 名;选中时间线片段时
					    显示「正在编辑:<片段名>」+「← 返回全局」,让用户始终知道在编辑什么。 */}
					{hasTimelineSelection ? (
						<div className="mb-3 px-1">
							<div className="text-[10px] font-medium uppercase tracking-[0.16em] text-slate-500">
								{t("inspector.editingCrumb")}
							</div>
							<div className="mt-1 flex items-center gap-2">
								<span className="h-2 w-2 shrink-0 rounded-full bg-[#34B27B]" />
								<span className="truncate text-sm font-semibold text-slate-100">
									{selectedRegionLabel ?? activeModeLabel}
								</span>
								{onClearTimelineSelection && (
									<button
										type="button"
										onClick={onClearTimelineSelection}
										className="ml-auto shrink-0 rounded-md border border-white/10 px-2 py-1 text-[11px] text-slate-400 transition-colors hover:bg-white/[0.06] hover:text-slate-200"
									>
										{t("inspector.backToGlobal")}
									</button>
								)}
							</div>
						</div>
					...
					{!hasTimelineSelection && (
						<Accordion type="multiple" value={[activePanelMode]} className="space-y-2">
							...
							{/* Inkast §2: 原「时间轴」面板的音频波形开关已移到底部时间线工具条
							    (TimelineEditor),消除 rail 与底部时间线的重名困惑。 */}
						</Accordion>
					)}
				</div>
			</div>
			...
			{showCropDropdown && cropRegion && onCropChange && (
				<>
					...
					/>
					<div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-[60] bg-[#0A0C0E] rounded-2xl shadow-2xl border border-white/10 p-8 w-[90vw] max-w-5xl max-h-[90vh] overflow-auto animate-in zoom-in-95 duration-200">
