...
interface AnnotationSettingsPanelProps {
	...
	onDuplicate?: () => void;
	onDelete: () => void;
	// Inkast §3 图片叠加:『画面』数值控件(位置/宽度/不透明度/居中)。
			...
			<div className="mb-3">
				...
				>
					...
					<TabsContent value="image" className="mt-0 space-y-4">
						...
						{/* Inkast §3 图片叠加:『画面』变换 + 不透明度(对照剪辑器 Demo)。 */}
						{(onPositionChange || onSizeChange) && (
							<div className="space-y-2.5">
								...
								<div className="grid grid-cols-3 gap-2">
										...
										<div className="flex items-center gap-1 rounded-md border border-white/10 bg-white/5 px-2 py-1">
											<input
												...
												onChange={(e) => {
													...
													onPositionChange({ x, y: annotation.position.y });
												}}
										...
										<div className="flex items-center gap-1 rounded-md border border-white/10 bg-white/5 px-2 py-1">
											<input
												...
												onChange={(e) => {
													...
													onPositionChange({ x: annotation.position.x, y });
												}}
												className="w-full bg-transparent text-right text-xs text-slate-200 outline-none [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none"
											/>
											<span className="text-[10px] text-slate-500">%</span>
									...
									<div className="space-y-1">
										...
										<div className="flex items-center gap-1 rounded-md border border-white/10 bg-white/5 px-2 py-1">
											<input
												type="number"
												value={Math.round(annotation.size.width)}
												onChange={(e) => {
													if (!onSizeChange) return;
													const oldW = annotation.size.width || 1;
													const newW = Math.max(5, Math.min(100, Number(e.target.value) || 0));
													const newH = Math.max(
														3,
														Math.min(100, (annotation.size.height || oldW) * (newW / oldW)),
													);
								...
								<Button
									...
									onClick={() => {
										onPositionChange?.({
											x: 50 - annotation.size.width / 2,
											y: 50 - annotation.size.height / 2,
										});
									}}
									className="w-full h-7 bg-white/5 text-slate-200 border-white/10 hover:bg-white/10 text-xs"
								>
									{t("overlay.center")}
								</Button>
							</div>
