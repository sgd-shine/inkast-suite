...
interface EditorEmptyStateProps {
	/** Inkast §2: 录制新视频 —— 把核心功能补成空状态的并列第一入口(打开录制源选择器)。 */
	onRecordNew: () => void;
...
export function EditorEmptyState({
	...
	return (
		<div
			className="flex h-full w-full flex-col items-center justify-center bg-[#0A0C0E]"
		...
		>
			...
			<div className="relative flex flex-col items-center gap-8 px-6 text-center">
				...
				{/* Actions — Inkast §2: 「录制新视频」补成并列第一入口(主行动绿),导入/打开退为次级。 */}
				<div className="flex flex-col gap-3 w-full max-w-xs">
					<button
						type="button"
						onClick={onRecordNew}
						className="flex items-center justify-center gap-2.5 w-full px-4 py-3 rounded-xl bg-[#34B27B] hover:bg-[#2d9e6c] active:bg-[#27885c] text-white font-medium text-sm transition-colors outline-none focus-visible:ring-2 focus-visible:ring-[#34B27B] focus-visible:ring-offset-2 focus-visible:ring-offset-[#0A0C0E]"
					>
						<Video className="h-4 w-4" />
						{te("emptyState.recordVideoButton")}
					</button>
					<button
						type="button"
