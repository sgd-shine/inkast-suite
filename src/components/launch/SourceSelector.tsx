...
export function SourceSelector() {
	...
	// Inkast: 框选区域录制 — drag-select a region on the chosen screen, then record as
	// usual. We capture the full display and the editor crops to the region. Only
	// meaningful for screen sources (a window source already defines its own bounds).
	const handleSelectRegion = async () => {
		if (!selectedSource || !selectedSource.id.startsWith("screen:")) return;
		const region = await window.electronAPI.openRegionSelector(selectedSource.display_id);
		if (!region) return; // cancelled — stay in the selector
		// The confirm path already stored the region; selecting the source proceeds to
		// the normal recording flow (HUD → record) and the editor applies the crop.
			...
			<div className="flex justify-center gap-2 border-t border-white/[0.06] p-3">
				...
				{selectedSource?.id.startsWith("screen:") && (
					<Button
						...
						className="h-8 rounded-lg px-5 text-[11px] text-zinc-300 transition-transform duration-150 hover:bg-white/5 hover:text-white active:scale-95"
					>
						{t("sourceSelector.selectRegion")}
					</Button>
				)}
				<Button
