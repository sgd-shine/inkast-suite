...
export function LaunchWindow() {
	...
	// Inkast: camera-overlay open state (synced via main, whether toggled by this
	// button or the ⌘⇧C shortcut) drives the HUD webcam button icon.
	const [cameraOverlayOn, setCameraOverlayOn] = useState(false);
	useEffect(() => {
		const off = window.electronAPI?.onCameraOverlayState?.((open) => setCameraOverlayOn(open));
		return () => off?.();
	}, []);

	// Inkast: 摄像头位置模式(录制前二选一)。fixed=成片用录制前位置;follow=还原录制中拖动轨迹。
	const [cameraPositionMode, setCameraPositionModeState] = useState<"fixed" | "follow">("fixed");
	useEffect(() => {
		window.electronAPI?.getCameraPositionMode?.().then((m) => {
			if (m === "fixed" || m === "follow") setCameraPositionModeState(m);
		});
	}, []);
	const toggleCameraPositionMode = useCallback(() => {
		setCameraPositionModeState((prev) => {
			const next = prev === "fixed" ? "follow" : "fixed";
			window.electronAPI?.setCameraPositionMode?.(next);
			return next;
		});
	}, []);

	// Inkast: teleprompter open state (toggled by this HUD button, the prompter's ✕,
	// or the ⌘⇧T shortcut) drives the HUD prompter button highlight.
	const [prompterOn, setPrompterOn] = useState(false);
	useEffect(() => {
		const off = window.electronAPI?.onPrompterState?.((open) => setPrompterOn(open));
		return () => off?.();
	}, []);

	useEffect(() => {
		let cancelled = false;
		void syncCameraPreviewWithWebcamRecorder({
			overlayOpen: cameraOverlayOn,
			webcamEnabled,
			recording,
			setWebcamEnabled,
			closeCameraOverlay: () => {
	...
	}, [cameraOverlayOn, webcamEnabled, recording, setWebcamEnabled]);
			...
			>
				...
				>
					...
					<button
						...
						className={`${hudIconBtnClasses} ${cameraOverlayOn ? "drop-shadow-[0_0_4px_rgba(74,222,128,0.4)]" : ""}`}
						onClick={() => {
							window.electronAPI?.toggleCameraOverlay?.();
						}}
						title={cameraOverlayOn ? t("webcam.disableWebcam") : t("webcam.enableWebcam")}
					>
						{cameraOverlayOn
							? getIcon("webcamOn", "text-green-400")
							: getIcon("webcamOff", "text-white/40")}
					</button>
					{/* 摄像头位置模式:开摄像头后才显示。固定=用录制前位置;跟随=还原录制中拖动。录制前选定。 */}
					{cameraOverlayOn && (
						<button
							type="button"
							data-testid="launch-camera-position-mode-button"
							className={`flex h-8 items-center rounded-md px-2 text-[10px] font-semibold transition-colors ${
								cameraPositionMode === "follow"
									? "bg-green-400/15 text-green-300"
									: "bg-white/10 text-white/70 hover:bg-white/15"
							} ${recording ? "cursor-not-allowed opacity-50" : ""}`}
							onClick={() => !recording && toggleCameraPositionMode()}
							disabled={recording}
							title={
								cameraPositionMode === "follow"
									? t("webcam.positionModeFollowHint")
									: t("webcam.positionModeFixedHint")
							}
						>
							{cameraPositionMode === "follow"
								? t("webcam.positionModeFollow")
								: t("webcam.positionModeFixed")}
						</button>
					)}
					...
					{/* Inkast: teleprompter — content-protected, not recorded. Works before
					    and during recording. ⌘⇧T also toggles it. */}
					<button
						data-testid="launch-prompter-button"
						className={`${hudIconBtnClasses} ${prompterOn ? "drop-shadow-[0_0_4px_rgba(74,222,128,0.4)]" : ""}`}
						onClick={() => window.electronAPI?.togglePrompter?.()}
						title={prompterOn ? t("prompter.close") : t("prompter.open")}
						aria-label={prompterOn ? t("prompter.close") : t("prompter.open")}
						aria-pressed={prompterOn}
					>
						<ScrollText
							size={ICON_SIZE}
							className={prompterOn ? "text-green-400" : "text-white/40"}
						/>
					</button>
				</div>
