...
import {
	beginCameraPositionCapture,
	endCameraPositionCapture,
...
// Inkast: region recording (框选区域录制) — the normalized crop region the user
// drag-selected before recording (null = full screen). The editor reads it on load
// via "get-selected-region" and applies it as the cropRegion, so the export is
// limited to that region even though we always capture the full display (方案 A).
// Reset whenever the source selector reopens, so it never leaks into the next take.
let selectedRegion: CropRegion | null = null;
export function setSelectedRegionValue(region: CropRegion | null): void {
...
export function registerIpcHandlers(
	...
	// Inkast: region recording — the editor reads this on load to crop to the
	// drag-selected region; clear is exposed so the editor can drop it if needed.
	ipcMain.handle("get-selected-region", () => {
		return selectedRegion;
	});

	ipcMain.handle("clear-selected-region", () => {
		selectedRegion = null;
		return { success: true };
	});
	...
	ipcMain.handle("open-source-selector", async () => {
		// Inkast: a new selection starts fresh — drop any region from a prior take so
		// a plain full-screen recording never inherits an earlier region crop.
		selectedRegion = null;
		...
		try {
			...
			const session: RecordingSession = webcamVideoPath
				? {
						...
						cursorCaptureMode,
						webcamSettings: getCameraOverlayRecordingSettings(),
					}
	...
	ipcMain.handle(
		...
		async (_, recording: boolean, recordingId?: number, cursorCaptureMode?: CursorCaptureMode) => {
			...
			// 摄像头位置采集与录制同生命周期(与光标遥测同 t=0):开始→快照+起采;结束→收尾。
			// 无摄像头时也无害(currentPosition 为空→时间线空,保存时不挂 webcamSettings)。
			if (recording) {
				beginCameraPositionCapture();
			} else {
				endCameraPositionCapture();
