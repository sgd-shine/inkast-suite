...
export function registerRecordingStreamHandlers(
	...
	// Injected (rather than imported) so this module — and its unit tests — stay
	// free of the WebM parser dependency chain.
	patchDurationOnDisk?: (filePath: string, durationMs: number) => Promise<unknown>,
): void {
	ipcMain.handle(
	...
	ipcMain.handle(
		...
		async (
			...
			try {
				// Flush and close the stream but KEEP the file — used when saving the
				// session failed, so the raw recording survives for manual recovery
				// instead of being deleted with the stream (close-recording-stream).
				...
				if (
					...
					// Best-effort: a preserved file without the WebM Duration header is
					// still playable, so a failed patch must not fail the preservation.
					await patchDurationOnDisk(resolveRecordingOutputPath(fileName), durationMs).catch(
						(error) => {
							console.warn(`[recording-stream] duration patch failed for ${fileName}:`, error);
						},
					);
				}
				return { success: true, streamed };
			} catch (error) {
				return { success: false, error: String(error) };
			}
		},
